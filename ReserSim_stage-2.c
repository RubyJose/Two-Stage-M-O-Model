...
int nVars = 22, nObjs = 2, nConsts = 1;
...
double EF_frac_dry		=	INPUT: "E-flow target fractions of dry year type"

double EF_frac_nor		=	INPUT: "E-flow target fractions of normal year type"

double EF_frac_wet		=	INPUT: "E-flow target fractions of wet year type"
...
for (jsim=0;jsim<nYear;jsim++)
{
	...
	for (isim=0; isim<12; isim++)
		{
		EfRelTar[isim][jsim]= EfRelTarMonth[isim][jsim]*EF_frac_dry[isim];			
		}	
} 
	...
	for (isim=0; isim<12; isim++)
		{
		EfRelTar[isim][jsim]= EfRelTarMonth[isim][jsim]*EF_frac_nor[isim];				
		} 	
	}
		...
		for (isim=0; isim<12; isim++)
		{		
		EfRelTar[isim][jsim]= EfRelTarMonth[isim][jsim]*EF_frac_wet[isim];
		} 
	}
...
if (overall_HA_HCA < HA-HCA)    // HA-HCA - HA value pertaining to the chosen E-flow target fractions from stage-1
{
	cstr[0] = 0;
...
{
	cstr[0] = overall_HA_HCA - HA-HCA;  // limiting constraint on HA to ensure its value being maintained within the value obtained from stage-1 corresponding to the selected E-flow target fractions 
