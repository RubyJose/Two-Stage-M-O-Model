...
void IHA_median_inf(int st_yr,int end_yr,int nYear, float Inflow[][366], float pre_PCT_Month[][3], float Nexp_Month[][3],
					float Median_Year[][12]){
...
{    
	...
	{    	
		...
		{	st_d  = ncldays[isim];
	 	...
	 	{
		 month_flow[ksim] =  Inflow[jsim][ksim];	 		 
		 }	
...
{	
	...
	percentile = Median_Monthly[pct1]+remain*(Median_Monthly[pct1+1]-Median_Monthly[pct1]);      
  	pre_PCT_Month[isim][0]=percentile;
...
// percentiles 	    	 
    ...
    pct1=pct1+st_yr-1;
	percentile = Median_Monthly[pct1]+remain*(Median_Monthly[pct1+1]-Median_Monthly[pct1]);      
  	pre_PCT_Month[isim][1]=percentile;	  
    ...
    pct1=pct1+st_yr-1;
	percentile = Median_Monthly[pct1]+remain*(Median_Monthly[pct1+1]-Median_Monthly[pct1]);      
  	pre_PCT_Month[isim][2]=percentile;  	  	  	 
}

// Expected number of years falling within each RVA category 
...
for(isim=0;isim<12;isim++){	
	...
	Nexp_Month[isim][0]=cnt1;
	Nexp_Month[isim][1]=cnt2;
	Nexp_Month[isim][2]=cnt3;
